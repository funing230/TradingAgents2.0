...
from tradingagents.dataflows.overnight_live_heavy_review_provider import (
    parse_heavy_review_scores,
    apply_heavy_selection,
)
from tradingagents.dataflows.overnight_live_provider import apply_multi_stage_review_fusion
...
def test_heavy_new_schema_materializes_default_watch_for_unmentioned_candidates():
    pool = _candidate_pool()
    text = """HEAVY_REVIEW_JSON_START
{
...
HEAVY_REVIEW_JSON_END"""
    result = parse_heavy_review_scores(text, pool)
...
def test_heavy_json_recovery_without_end_marker_uses_structured_reviews():
    pool = _candidate_pool()
    text = """HEAVY_REVIEW_JSON_START
{
  "trade_date": "2026-05-14",
...
def test_heavy_ranked_text_fallback_prefers_last_final_list():
    pool = _candidate_pool()
    text = """
...
"""
    ...
    selected = apply_heavy_selection(pool, result.scores, target_top_n=3)
    assert set(selected["ts_code"]).issubset({"000004.SZ", "000003.SZ", "000001.SZ", "000006.SZ", "000002.SZ"})


def test_multi_stage_fusion_applies_heavy_and_light_veto(tmp_path):
    scored = pd.DataFrame(
        {
    ...
    heavy_path = tmp_path / "heavy.csv"
    light_path = tmp_path / "light.csv"
    heavy.to_csv(heavy_path, index=False)
    light.to_csv(light_path, index=False)

    fused = apply_multi_stage_review_fusion(scored, heavy_path, light_path)
