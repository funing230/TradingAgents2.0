...
Tushare feature sources that are useful for an overnight-holding strategy:

- stock_basic
- daily_basic
- moneyflow
- stk_factor
- optional same-day 14:30-15:00 minute-window features
...
from tradingagents.dataflows.overnight_minute_cache import (
    DEFAULT_MINUTE_CACHE_DIR,
    fetch_minute_window_frame,
    summarize_minute_features,
)
from tradingagents.dataflows.tushare_provider import _fmt_date, _get_pro, _parse_date
...
def add_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    total_mv_series = pd.to_numeric(out["total_mv"], errors="coerce") if "total_mv" in out.columns else pd.Series(pd.NA, index=out.index, dtype="float64")
    circ_mv_series = pd.to_numeric(out["circ_mv"], errors="coerce") if "circ_mv" in out.columns else pd.Series(pd.NA, index=out.index, dtype="float64")
    ...
    out["net_mf_amount"] = pd.to_numeric(out.get("net_mf_amount"), errors="coerce")
    amount_t = pd.to_numeric(out.get("amount_t"), errors="coerce")
...
def write_audit(path: Path, labels: pd.DataFrame, features: pd.DataFrame, topn: pd.DataFrame, errors: list[str], top_n: int, source_labels: Path) -> None:
...
def main() -> None:
    ...
    parser.add_argument("--force-refresh", action="store_true", help="Ignore cached API CSVs")
    ...
    write_audit(audit_path, labels, scored, topn, errors, args.top_n, labels_path)
